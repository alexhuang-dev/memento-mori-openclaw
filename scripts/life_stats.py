...
Commands:
  ...
  python scripts/life_stats.py share --format text
...
CHECKIN_STYLES = {"stoic", "gentle", "sharp", "poetic", "minimal", "terse"}
STOPWORDS = {
    "the",
    "and",
    "that",
...
def latest_journal_entry(journal: dict[str, Any], preferred_week: str | None = None) -> tuple[str | None, dict[str, Any]]:
    if preferred_week and preferred_week in journal:
        return preferred_week, normalized_entry(journal[preferred_week])
    weeks = sorted_weeks(journal)
    if not weeks:
        return None, normalized_entry({})
    week = weeks[-1]
    return week, normalized_entry(journal[week])


def checkin_opening(style: str, stats: dict[str, Any]) -> str:
    style = style if style in CHECKIN_STYLES else "stoic"
    weeks_lived = stats["weeks_lived"]
    weeks_left = stats["weeks_left"]
    if style == "sharp":
        return (
            f"第 {weeks_lived:,} 周结束了。\n\n"
            "你没有失去一天。\n"
            "你失去的是这一整周。\n\n"
            "要留下些什么吗？"
        )
    ...
    if style == "poetic":
        return (
            f"第 {weeks_lived:,} 周落下来了。\n\n"
            f"还剩大约 {weeks_left:,} 周，\n"
            "像一排没有写字的格子。\n\n"
            "这一格，要留下什么？"
        )
    if style in {"minimal", "terse"}:
        return f"第 {weeks_lived:,} 周。还剩 {weeks_left:,} 周。留下些什么？"
    return (
        f"第 {weeks_lived:,} 周。\n\n"
        f"还剩 {weeks_left:,} 周。\n\n"
        "这一周是否值得记录？"
...
def cmd_checkin(args: dict[str, Any]) -> None:
    ...
    if args.get("mark_milestones", True):
        ...
        save(state)
    style = str(args.get("style", state.get("config", {}).get("checkin_style", "stoic")))
    emit(
        {
            **stats,
            "new_milestones": new_milestones,
            "checkin_style": style,
            "suggested_opening": checkin_opening(style, stats),
            "recent_journal": recent_journal(state.get("journal", {})),
            "last_checkin": state.get("last_checkin"),
            "total_checkins": state.get("total_checkins", 0),
            "config": state.get("config", {}),
...
def cmd_config(args: dict[str, Any]) -> None:
    ...
    for key in ("checkin_style", "preserve_raw_entries", "default_life_expectancy_years"):
        if key in args:
            if key == "checkin_style" and str(args[key]) not in CHECKIN_STYLES:
                fail("invalid_checkin_style", f"checkin_style must be one of: {', '.join(sorted(CHECKIN_STYLES))}.")
            config[key] = args[key]
...
def share_text(stats: dict[str, Any], week: str | None, entry: dict[str, Any]) -> str:
    summary = entry_summary(entry) or "这一周还没有留下文字。"
    week_line = f"本周记录：{week}" if week else "本周记录：—"
    return "\n".join(
        [
            "我的人生周历",
            "",
            f"已走过：{stats['weeks_lived']:,} 周",
            f"还剩下：{stats['weeks_left']:,} 周",
            "",
            f"{stats['progress_bar']} {stats['pct_done_display']}%",
            "",
            week_line,
            "",
            "这一周，我留下的是：",
            f"“{summary}”",
            "",
            "Generated by Memento Mori for OpenClaw",
        ]
    )


def share_svg(stats: dict[str, Any], week: str | None, entry: dict[str, Any]) -> str:
    summary = html.escape(entry_summary(entry) or "这一周还没有留下文字。")
    week_label = html.escape(week or "—")
    filled = round(stats["pct_done_display"] / 100 * 28)
    cells = []
    for index in range(28):
        x = 70 + (index % 14) * 24
        y = 72 + (index // 14) * 24
        color = "#D7C6A1" if index < filled else "#344149"
        opacity = "0.95" if index < filled else "0.55"
        cells.append(f'<rect x="{x}" y="{y}" width="14" height="14" rx="3" fill="{color}" opacity="{opacity}"/>')
    return f'''<svg width="900" height="520" viewBox="0 0 900 520" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="title desc">
  <title id="title">Memento Mori life card</title>
  <desc id="desc">A share card showing weeks lived, weeks left, and this week's remembered sentence.</desc>
  <rect width="900" height="520" rx="24" fill="#101418"/>
  <rect x="1" y="1" width="898" height="518" rx="24" stroke="#2F3A40" stroke-width="2"/>
  {''.join(cells)}
  <text x="70" y="180" fill="#F5EFE0" font-family="Georgia, Times New Roman, serif" font-size="42">我的人生周历</text>
  <text x="70" y="224" fill="#D7C6A1" font-family="Inter, Segoe UI, Arial, sans-serif" font-size="20">已走过 {stats['weeks_lived']:,} 周 · 还剩 {stats['weeks_left']:,} 周</text>
  <rect x="70" y="258" width="520" height="16" rx="8" fill="#263139"/>
  <rect x="70" y="258" width="{round(stats['pct_done_display'] / 100 * 520)}" height="16" rx="8" fill="#D7C6A1"/>
  <text x="612" y="273" fill="#E7E0CE" font-family="Inter, Segoe UI, Arial, sans-serif" font-size="18">{stats['pct_done_display']}%</text>
  <text x="70" y="330" fill="#92A2AA" font-family="Inter, Segoe UI, Arial, sans-serif" font-size="16">这一周，我留下的是：</text>
  <text x="70" y="372" fill="#F5EFE0" font-family="Georgia, Times New Roman, serif" font-size="28">“{summary[:42]}”</text>
  <text x="70" y="430" fill="#77878F" font-family="Inter, Segoe UI, Arial, sans-serif" font-size="15">Week {week_label} · Generated by Memento Mori for OpenClaw</text>
</svg>
'''


def cmd_share(args: dict[str, Any]) -> None:
    state = load()
    profile = require_profile(state)
    stats = compute(profile["birthdate"], int(profile.get("life_expectancy_years", 85)))
    preferred_week = args.get("week") or stats["current_week"]
    week, entry = latest_journal_entry(state.get("journal", {}), str(preferred_week) if preferred_week else None)
    fmt = str(args.get("format", "text")).lower()
    if fmt == "text":
        output = share_text(stats, week, entry) + "\n"
    elif fmt == "svg":
        output = share_svg(stats, week, entry)
    else:
        fail("invalid_format", "share format must be text or svg.")
